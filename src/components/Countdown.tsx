...
interface CountdownProps {
  targetDate: string; // ISO string — Date não é serializável de server→client
}
...
export default function Countdown({ targetDate }: CountdownProps) {
  ...
  useEffect(() => {
    ...
    const calc = () => setDiff(target - Date.now());
    calc();
    const id = setInterval(calc, 60_000);
    return () => clearInterval(id);
  }, [targetDate]);

  // Placeholder estável durante SSR / antes da hidratação
  if (diff === null) {
    ...
    <div className="flex gap-3 justify-center">
      {[
        ...
        <div key={label} className="flex items-center gap-3">
          ...
          {i < 2 && (
            <span className="font-display text-2xl mb-5" style={{ color: "var(--text-muted)" }}>
              :
            </span>
          )}
        </div>
